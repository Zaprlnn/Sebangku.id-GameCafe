...
export default function CategoriesTablesView() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: tableData } = await supabase.from('cafe_tables').select('*').order('table_no', { ascending: true });
  ...
  const openEditTable = (t: CafeTable) => {
    setTableId(t.id); setTableName(t.table_no); setTableCapacity(t.capacity); setTableLocation(t.location || ""); setTableStatus(t.status); setShowTableModal(true);
  };
    ...
    try {
      if (tableId) {
        await supabase.from('cafe_tables').update({ 
          table_no: tableName, capacity: tableCapacity, location: tableLocation, status: tableStatus 
        }).eq('id', tableId);
      } else {
                      ...
                      <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                        <td className="px-5 py-3.5 font-bold text-[#0F172A]">
                          <div className="flex items-center gap-2">
                            <Hash size={14} className="text-slate-400" /> {t.table_no}
                          </div>
                        </td>
